class SomeThing
  ...
  def self.do_thing(input)
    ...
    h = {}
    sleep_by_guard.each do |guard_number, sleep_array|
      sleepiest_minute = sleep_array.max_by { |i| sleep_array.count(i) }
      h[sleep_array.count(sleepiest_minute)] = [guard_number, sleepiest_minute]
    end
    frequency, info = h.max_by { |k, v| k }
    info[0].to_i * info[1]
  end
end
...
class SomeThingTest < Minitest::Test
  def test_case1
    ...
    assert_equal 4455, SomeThing.do_thing(input)
  end
end
